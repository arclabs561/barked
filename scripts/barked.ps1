...
function Print-Header {
    ...
    Write-Host "          BARKED HARDENING WIZARD v$($script:ToolVersion)          " -ForegroundColor Green -NoNewline
    Write-ColorLine "║" Green
    Write-Host "║" -ForegroundColor Green -NoNewline
...
function Write-AuditReport {
    ...
    $report += ""
    $report += "---"
    $report += "Generated by barked.ps1 v$($script:ToolVersion)"
    try {
...
function Write-DryRunReport {
    ...
    $report += ""
    $report += "---"
    $report += "Generated by barked.ps1 v$($script:ToolVersion) (dry-run mode)"
...
function Invoke-Clean {
    ...
    Write-ColorLine "╔══════════════════════════════════════════════════╗" Green
    Write-Host "║" -ForegroundColor Green -NoNewline
    Write-Host "          BARKED SYSTEM CLEANER v$($script:ToolVersion)           " -ForegroundColor Green -NoNewline
    Write-ColorLine "║" Green
    Write-Host "║" -ForegroundColor Green -NoNewline
    Write-Host "                     Windows                       " -NoNewline
    Write-ColorLine "║" Green
...
function Invoke-Update {
    ...
    if (-not (Test-VersionGt -New $latest -Current $script:ToolVersion)) {
        Write-ColorLine "Already up to date (v$($script:ToolVersion))." Green
        exit 0
    }
...
function Invoke-PassiveUpdateCheck {
    ...
    try {
        if (Test-Path $cacheFile) {
            ...
            if ($lines.Count -ge 2) {
                ...
                if (($now - $cachedEpoch) -lt $cacheMax) {
                    if ($cachedVersion -and (Test-VersionGt -New $cachedVersion -Current $script:ToolVersion)) {
        ...
        if (Test-VersionGt -New $latest -Current $script:ToolVersion) {
...
function Print-Help {
    ...
    Write-Host "  .\barked.ps1 -DryRun                    Preview hardening changes"
    Write-Host "  .\barked.ps1 -Auto -Profile high -DryRun Preview high-profile changes"
    Write-Host ""
...
function Main {
    ...
    if ($Version) {
        Write-Host "barked v$($script:ToolVersion)"
        exit 0
    }
