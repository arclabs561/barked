...
write_dry_run_report() {
...

    {
        ...
        echo ""
        echo "---"
        echo "Generated by harden.sh v${VERSION} (dry-run mode)"
    } > "$report_file"
...
# Human-readable description of what a module would do in dry-run
dry_run_description() {
    case "$1" in
        disk-encrypt)          echo "Verify disk encryption (manual)" ;;
        firewall-inbound)      echo "Enable system firewall, block all incoming" ;;
        firewall-stealth)      echo "Enable stealth mode / drop ICMP" ;;
        firewall-outbound)     echo "Install outbound firewall (LuLu / ufw)" ;;
        dns-secure)            echo "Configure encrypted DNS (Quad9)" ;;
        vpn-killswitch)        echo "Enable VPN always-on kill switch" ;;
        hostname-scrub)        echo "Set generic hostname" ;;
        mac-rotate)            echo "Enable MAC address rotation" ;;
        telemetry-disable)     echo "Disable OS and browser telemetry" ;;
        traffic-obfuscation)   echo "Configure DAITA / Tor guidance" ;;
        metadata-strip)        echo "Install exiftool / mat2 metadata tools" ;;
        browser-basic)         echo "Harden Firefox: block trackers, HTTPS-only" ;;
        browser-fingerprint)   echo "Enable fingerprint resistance in Firefox" ;;
        guest-disable)         echo "Disable guest account" ;;
        lock-screen)           echo "Set immediate lock screen password" ;;
        bluetooth-disable)     echo "Disable Bluetooth when unused" ;;
        git-harden)            echo "Configure SSH signing and credential helper" ;;
        dev-isolation)         echo "Set up Docker/VM isolation guidance" ;;
        ssh-harden)            echo "Generate Ed25519 keys, strict SSH config" ;;
        monitoring-tools)      echo "Install Objective-See / auditd+aide" ;;
        permissions-audit)     echo "Audit granted permissions" ;;
        audit-script)          echo "Schedule weekly automated audit" ;;
        auto-updates)          echo "Enable automatic security updates" ;;
        backup-guidance)       echo "Encrypted backup strategy guidance" ;;
        border-prep)           echo "Travel protocol and nuke checklist" ;;
        kernel-sysctl)         echo "Harden kernel parameters via sysctl.d" ;;
        apparmor-enforce)      echo "Set AppArmor to enforce / audit App Sandbox" ;;
        boot-security)         echo "Verify Secure Boot / SIP, set GRUB password" ;;
        *)                     echo "Apply ${1} hardening" ;;
    esac
}

# Write dry-run report to file
write_dry_run_report() {
    local report_dir="${SCRIPT_DIR}/../audits"
    mkdir -p "$report_dir"
    local report_file="${report_dir}/dry-run-${DATE}.md"

    {
        ...
        echo "**Generated:** ${TIMESTAMP}"
        echo ""
        echo "## Would-Be Changes"
        echo ""
        echo "| Module | Current | Planned | Severity |"
        echo "|--------|---------|---------|----------|"
        for entry in "${LOG_ENTRIES[@]}"; do
            echo "| ${entry} |"
        done
        ...
        echo ""
        echo "---"
        echo "Generated by harden.sh v${VERSION} (dry-run)"
    } > "$report_file"
