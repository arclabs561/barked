...
./scripts/barked.sh --clean-schedule   # Set up scheduled cleaning
./scripts/barked.sh --clean-unschedule # Remove scheduled cleaning
./scripts/barked.sh --update       # Self-update
./scripts/barked.sh --version      # Show version
...
Two main scripts with full feature parity (audit, clean, scheduled cleaning, auto mode, quiet mode, dry-run, uninstall, modify, update, profiles). Only platform-specific modules and clean targets differ:
- `scripts/barked.sh` — Bash 4+, ~7,400 lines, v2.2.2
- `scripts/barked.ps1` — PowerShell 5+, ~5,090 lines, v1.4.1
...
1. Choose a kebab-case ID (e.g. `my-feature`)
2. Implement four functions: `check_my_feature()`, `apply_my_feature()`, `verify_my_feature()`, `revert_my_feature()`
3. Add the module ID to the `ALL_MODULES` array and the relevant profile arrays (`STANDARD_MODULES`, `HIGH_MODULES`, `PARANOID_MODULES`)
4. Add OS-specific branches inside each function as needed
5. Mirror the implementation in `barked.ps1` for Windows parity
...
Feature branches use git worktrees at `.worktrees/<feature-name>/`. No active worktrees.
...
Releases via `gh release create` with both scripts **and their `.sha256` checksum files** as assets. Update and install flows verify downloads against these checksums — releases without them will break the install pipeline. See README for the full release checklist.

## Gotchas

- **Bash 4+ required on macOS** — ships with 3.2; users need `brew install bash`. Script uses associative arrays (`declare -A`).
- **Auto mode available** — both scripts support `-Auto`/`--auto` with `-Profile`/`--profile` for non-interactive hardening, and `-Quiet`/`--quiet` to suppress console output.
